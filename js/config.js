...
config_index = 2 // use which entry in the config file; for 2019 it is 1
...
d3.json("/config/config.json", function (err, config) {
    ...
    // which year? 
    d3.selectAll(".viscurlogo")
        .attr("link:href", config_info.vis_cur_logo_link)
    
    // which year for the contest? 
    d3.selectAll(".shirt_year")
        .text(config_info.shirt_year)
